...
void	ft_convert(t_printf *my_printf, va_list printf_list, char *str)
{
	...
	printf("ft_convert.c: char is %c\n", str[my_printf->i]);
	int n = ft_strchr_bool(PARSE, str[my_printf->i]);
	//printf("%d\n", n);
			...
			(ft_strchr_bool(PARSE, str[my_printf->i]) == 0))
	{
		...
		my_printf->i += 1;
		printf("ft_convert.c: section1: char is now %c\n", str[my_printf->i]);
	}
	if (ft_strchr_bool(PARSE, str[my_printf->i]))
	{
		printf("ft_convert.c: section2: char is now %c\n", str[my_printf->i]);
